<template>
    <div class="wizard-step-preguntas-solicitante">
        <h3 class="text-xl font-semibold text-gray-900 mb-6">
            <i class="fas fa-user text-blue-600 mr-2"></i>
            Preguntas del Formulario Solicitante
        </h3>

        <div class="space-y-6 mb-8">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">
                    Nombre del cuestionario solicitud <span class="text-red-500">*</span>
                </label>
                <input
                    :value="formData.questionnaire_solicitante.name"
                    @input="updateField('name', $event.target.value)"
                    type="text"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="Ej: Bono Alquiler Joven Cataluña solicitud"
                />
            </div>
            ...
            <div>
                <label class="flex items-center">
                    <input
                        :checked="formData.questionnaire_solicitante.active"
                        @change="updateField('active', $event.target.checked)"
                        type="checkbox"
                        class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50"
                    />
                    <span class="ml-2 text-sm text-gray-700">Cuestionario activo</span>
                </label>
            </div>
        </div>
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between">
                                    ...
                                    <div class="flex items-center space-x-1 ml-4 flex-shrink-0">
                                        ...
                                        <button
                                            @click.stop="$emit('remove-question', index)"
...
// Estado de drag and drop local
const isDragOver = computed(() => props.dragOverIndex !== null)

// Cuando se añadan/quiten categorías o finalidades, mostrar automáticamente
// el listado de preguntas filtradas (sin necesidad de escribir texto)
watch([collectorCategoryFilter, collectorPurposeFilter], () => {
    if (
    ...
    // Siempre que haya alguna categoría o finalidad seleccionada (o texto),
    // lanzamos la búsqueda para mostrar el listado de preguntas
    searchCollectorQuestions()
})

watch([generalCategoryFilter, generalPurposeFilter], () => {
    if (
...
const updateField = (field, value) => {
    emit('update:formData', {
        ...
        questionnaire_solicitante: {
            ...props.formData.questionnaire_solicitante,
            [field]: value,
        },
    })
}
