...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 font-sans text-gray-900 min-h-screen flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-2xl bg-white rounded-2xl overflow-hidden floating-card">
        ...
        <!-- Form Container -->
        <div class="p-6">
        <form id="questionnaireForm" action="/register-user" method="POST">
            @csrf
    ...
    <script>
        ...
        function checkAndSkipQuestion() {
            const question = @json($questions)[currentQuestion]; 
            const userAnswer = formData[`answers[${question.id}][value]`];

            if (question.pivot.condition && userAnswer == question.pivot.condition) {
                const nextQuestionIndex = findNextQuestionIndex(question.pivot.next_question_id);
                if (nextQuestionIndex !== -1) {
                    var test = document.getElementById(`question_${currentQuestion}`);
                    if (test) test.classList.add('invisible');
                    historyStack.push(currentQuestion);
                    currentQuestion = nextQuestionIndex;
                    changeQuestion('next', nextQuestionIndex, true);
                    return true;
                }
            }
            return false;
        ...
        function findNextQuestionIndex(nextQuestionId) {
            return @json($questions).findIndex(q => q.id === nextQuestionId);
        }


        function changeQuestion(direction, nextQuestion = null, skipPush = false) {
            if (isAnimating) return;
            isAnimating = true;
            const currentElement = document.getElementById(`question_${currentQuestion}`);

            if (direction === 'next' && !skipPush) {
                historyStack.push(currentQuestion);
                console.log('Push automático en changeQuestion:', currentQuestion);
            }
            ...
            setTimeout(() => {
                ...
                if (nextQuestion !== null) {
                    newQuestionIndex = nextQuestion;
                } else if (direction === 'next') {
                    newQuestionIndex = currentQuestion + 1;
                } else if (direction === 'prev') {
                    const lastQuestion = historyStack.pop();
                    newQuestionIndex = lastQuestion !== undefined ? lastQuestion : 0;
                }
        ...
        nextBtn.addEventListener('click', () => {
            saveCurrentAnswer();
            const skipped = checkAndSkipQuestion();
        ...
        submitBtn.addEventListener('click', () => {
            ...
            formData.hizo_collector_real = false;
            axios.post('https://sandboxapp.tutramitefacil.es/register-user', formData)
